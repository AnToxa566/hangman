...
const WordPattern: React.FC<Props> = ({ phrase, openLetters }) => {
  ...
  const isLetterOpen = (letter: string): boolean => Boolean(openLetters.find((lt) => lt.toLowerCase() === letter.toLowerCase()));

  const getSymbol = (letter: string): string => isLetterOpen(letter) ? letter.toUpperCase() : '_';
